...
def _expand_mask(mask, dtype, tgt_len=None):
    """
    旧版 Hugging Face transformers 中的 `_expand_mask` 实现。
    适配新版 transformers 中已移除的 API。
    """
    import torch
    ...
    if mask.dim() == 2:
        ...
        raise ValueError(f"Wrong shape for mask (shape {mask.shape})")

    expanded_mask = expanded_mask.to(dtype=dtype)

    if tgt_len is not None:
        expanded_mask = expanded_mask[:, :, :tgt_len, :]
