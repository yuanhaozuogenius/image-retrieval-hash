...
class BLIP_HashWrapper(nn.Module):
    def __init__(self, bit, blip_ckpt='./models/model_base.pth', med_config='BLIP/configs/med_config.json'):
        ...
        # 替换 vision_proj 为目标维度（比如64维）
        hidden_size = self.model.visual_encoder.num_features  # 一般是768
        self.model.vision_proj = nn.Linear(hidden_size, bit)

        # 可选：初始化
        nn.init.normal_(self.model.vision_proj.weight, std=0.02)
        nn.init.constant_(self.model.vision_proj.bias, 0)

        # 冻结所有 BLIP 参数（包括 encoder）
