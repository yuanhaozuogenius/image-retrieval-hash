...
class MyDataset(nn.Module):
    def __init__(self, data_path, data_name, txt, transform=None, target_transform=None):
        ...
        self.images = list(map(lambda x: data_path+'/'+x, images))
        self.labels = labels
...
def init_dataloader(data_path, data_name, train_list, database_list, test_list, batchSize):
    ...
    if data_name == 'cifar100':
        ...
        train_dataset = CIFAR10(data_path, train=True, transform=transform_1, target_transform=one_hot(10),
                                download=False)
        database_dataset = CIFAR10(data_path, train=True, transform=transform_1, target_transform=one_hot(10))
        test_dataset = CIFAR10(data_path, train=False, transform=transform_2, target_transform=one_hot(10))
    else:
    ...
    database_loader = DataLoader(database_dataset, batch_size=batchSize, num_workers=0, pin_memory=True)
    test_loader = DataLoader(test_dataset, batch_size=batchSize, num_workers=0, pin_memory=True)
