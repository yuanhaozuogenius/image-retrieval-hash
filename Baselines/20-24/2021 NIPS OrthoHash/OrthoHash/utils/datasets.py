...
class HashingDataset(Dataset):
    def __init__(self,
        ...
        self.train_labels = np.array(self.train_labels, dtype=np.float64)
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(self.image_root, img)  # ✅ 改成真实图片根目录
...
def cifar(nclass, **kwargs):
    ...
    traind = CIFAR(root=cifar10_dir,
                   transform=transform, target_transform=one_hot(int(nclass)),
                   train=True, download=false)
    testd = CIFAR(root=cifar10_dir, train=False, download=false)
    ...
    if not load_data:
        ...
        data_index = torch.load(path, weights_only=False)# PyTorch 2.6 开始 torch.load 默认 weights_only=True，
        # 而你的索引文件（虽然后缀是 .txt，但其实是用 torch.save 存的 pickle 文件）里包含了 NumPy 的对象，
        # 按默认安全名单会被拒绝加载。所以需要把 torch.load(path) 改成 torch.load(path, weights_only=False)
...
def coco(**kwargs):
    ...
    config = kwargs.get('config', None)  # 新增：从参数里读取config
    root_path = DATA_FOLDER['coco']


    if config is not None and "image_root" in config:
        root_path = config["image_root"]

    d = HashingDataset(DATA_FOLDER['coco'], transform=transform, filename=filename, image_root=r"D:\Datasets\coco2017")
    return d
