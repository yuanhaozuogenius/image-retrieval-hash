...
运行run_coco.sh  
## 250917 cmd
`python main.py --ds coco --nbit 64 --bs 64 --epochs 100 --codebook-method B --margin 0.3 --seed 59495 --eval_interval 10 --R 5000`

1. 在`main.py`中新增 config["txt_root"] = os.path.join("data", config["dataset"]) config["image_root"] = r"D:\Datasets\coco2017"  
2. 在`datasets.py`修改HashingDataset,新增一个参数image_root,并修改对应调用处即可。  
3. 在`config.py`修改topk求map
4. run 中添加`--ds coco --nbit 64 --bs 64 --epochs 100 --codebook-method B --margin 0.3 --seed 59495`
